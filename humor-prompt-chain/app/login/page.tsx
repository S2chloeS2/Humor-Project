...
function LoginContent() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center" style={{ backgroundColor: "var(--bg-base)" }}>
      <div
        className="rounded-xl p-10 w-full max-w-sm text-center animate-fade-up"
        style={{
      ...
      >
        ...
        <div className="flex items-center justify-center gap-2 mb-6">
          ...
          <span className="font-bold font-mono text-sm tracking-widest shimmer-text">PROMPT CHAIN</span>
        </div>
        ...
        {error === "unauthorized" && (
          <div
            className="mb-6 p-3 rounded-lg text-sm text-left"
            style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.3)", color: "var(--danger)" }}
          >
            Access denied — superadmin or matrix admin role required.
          </div>
        )}

        <button
          onClick={handleLogin}
          className="w-full py-3 px-6 font-semibold rounded-lg transition-all flex items-center justify-center gap-2 hover:opacity-90 active:scale-95"
          style={{ backgroundColor: "var(--accent)", color: "#0f172a" }}
        >
          <svg width="16" height="16" viewBox="0 0 18 18" fill="none">
            <path d="M17.64 9.205c0-.639-.057-1.252-.164-1.841H9v3.481h4.844a4.14 4.14 0 0 1-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615Z" fill="#1a1a1a"/>
